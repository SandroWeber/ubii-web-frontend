/**
 * @author kaisalmen / https://github.com/kaisalmen
 *
 * Based on @kaisalmen's WWOBJLoader
...
Callbacks.prototype = {
     ...
     * Use {@link LoadedMeshUserOverride} for alteration instructions (geometry, material or disregard mesh).
     *
 ...
 */
var LoadedMeshUserOverride = function (disregardMesh, alteredMesh) {
...
/**
 * A resource description used by {@link PrepData} and others.
 * @class
 *
 * @param {string} url URL to the file
 * @param {string} extension The file extension (type)
 */
var ResourceDescriptor = function (url, extension) {
    ...
    if (urlParts.length >= 2) {
        ...
        this.path = Validator.verifyInput(urlParts.slice(0, urlParts.length - 1).join('/') + '/', this.path);
 ...
 * @class
 */
var PrepData = function (modelName) {
    this.logging = {
        enabled: true,
        debug: false
    };
...
PrepData.prototype = {
    ...
    /**
     * Returns all callbacks as {@link Callbacks}
     *
     * @returns {Callbacks}
     */
     ...
     *
     * @param {ResourceDescriptor} Adds a {@link ResourceDescriptor}
     */
     ...
     *
     * @returns {@link PrepData}
     */
    clone: function () {
        var clone = new PrepData(this.modelName);
    ...
    /**
     * Identify files or content of interest from an Array of {@link ResourceDescriptor}.
     *
     * @param {ResourceDescriptor[]} resources Array of {@link ResourceDescriptor}
     * @param Object fileDesc Object describing which resources are of interest (ext, type (string or UInt8Array) and ignore (boolean))
     * @returns {{}} Object with each "ext" and the corresponding {@link ResourceDescriptor}
     */
...
var MeshBuilder = function () {
    console.info('Using MeshBuilder version: ' + MeshBuilder.LOADER_MESH_BUILDER_VERSION);
...
MeshBuilder.LOADER_MESH_BUILDER_VERSION = '1.3.0';
...
var WorkerSupport = function () {
    ...
    this.loaderWorker = typeof window !== "undefined" ? new WorkerSupport.LoaderWorker() : new WorkerSupport.NodeLoaderWorker();
};

WorkerSupport.WORKER_SUPPORT_VERSION = '2.3.0';
...
WorkerSupport.prototype = {
     ...
     * @param {WorkerRunnerRefImpl} runnerImpl The default worker parser wrapper implementation (communication and execution). An extended class could be passed here.
     */
    validate: function (functionCodeBuilder, parserName, libLocations, libPath, runnerImpl) {
        ...
        if (Validator.isValid(runnerImpl)) {
            ...
            if (this.logging.enabled) console.info('WorkerSupport: Using DEFAULT "WorkerRunnerRefImpl" as Runner class for worker.');

            // NodeJS implementation
        } else {

            runnerImpl = NodeWorkerRunnerRefImpl;
            if (this.logging.enabled) console.info('WorkerSupport: Using DEFAULT "NodeWorkerRunnerRefImpl" as Runner class for worker.');

        }
        var userWorkerCode = functionCodeBuilder(WorkerSupport.CodeSerializer);
        userWorkerCode += 'var Parser = ' + parserName + ';\n\n';
        userWorkerCode += WorkerSupport.CodeSerializer.serializeClass(runnerImpl.runnerName, runnerImpl);
     ...
     *
     * @param {Function} meshBuilder The mesh builder function. Default is {@link MeshBuilder}.
...
WorkerSupport.LoaderWorker = function () {
    this._reset();
};

WorkerSupport.LoaderWorker.prototype = {

    constructor: WorkerSupport.LoaderWorker,
 ...
 * @class
 */
var WorkerRunnerRefImpl = function () {
...
WorkerRunnerRefImpl.runnerName = 'WorkerRunnerRefImpl';

WorkerRunnerRefImpl.prototype = {

    constructor: WorkerRunnerRefImpl,
 ...
 * @class
 * @extends WorkerRunnerRefImpl
 */
var NodeWorkerRunnerRefImpl = function () {
    this.runnerName = 'NodeWorkerRunnerRefImpl';
...
NodeWorkerRunnerRefImpl.prototype = Object.create(WorkerRunnerRefImpl.prototype);
NodeWorkerRunnerRefImpl.prototype.constructor = NodeWorkerRunnerRefImpl;
NodeWorkerRunnerRefImpl.runnerName = 'NodeWorkerRunnerRefImpl';

NodeWorkerRunnerRefImpl.prototype = {
 ...
 */
WorkerSupport.NodeLoaderWorker = function () {
    WorkerSupport.LoaderWorker.call(this);
};

WorkerSupport.NodeLoaderWorker.prototype = Object.create(WorkerSupport.LoaderWorker.prototype);
WorkerSupport.NodeLoaderWorker.prototype.constructor = WorkerSupport.NodeLoaderWorker;

/**
 * @inheritdoc
 */
WorkerSupport.NodeLoaderWorker.checkSupport = function () {
    try {
        // Work around webpack builds failing with NodeJS requires
        var _require = eval('require');
 ...
 * @inheritdoc
 */
WorkerSupport.NodeLoaderWorker.prototype.initWorker = function (code, runnerImplName) {
    var supportError = this.checkSupport();
    if (supportError) {
 ...
 */
var WorkerDirector = function (classDef) {
    console.info('Using WorkerDirector version: ' + WorkerDirector.LOADER_WORKER_DIRECTOR_VERSION);
    this.logging = {
        enabled: true,
        debug: false
    };

    this.maxQueueSize = WorkerDirector.MAX_QUEUE_SIZE;
    this.maxWebWorkers = WorkerDirector.MAX_WEB_WORKER;
...
WorkerDirector.LOADER_WORKER_DIRECTOR_VERSION = '2.3.0';
WorkerDirector.MAX_WEB_WORKER = 16;
WorkerDirector.MAX_QUEUE_SIZE = 2048;

WorkerDirector.prototype = {
    ...
    prepareWorkers: function (globalCallbacks, maxQueueSize, maxWebWorkers) {
        ...
        this.maxQueueSize = Math.min(maxQueueSize, WorkerDirector.MAX_QUEUE_SIZE);
        this.maxWebWorkers = Math.min(maxWebWorkers, WorkerDirector.MAX_WEB_WORKER);
        ...
        for (var instanceNo = 0; instanceNo < this.maxWebWorkers; instanceNo++) {
            ...
            var workerSupport = new WorkerSupport();
    ...
    _kickWorkerRun: function (prepData, supportDesc) {
        ...
        var updatedCallbacks = new Callbacks();
    ...
    _buildLoader: function (instanceNo) {
        ...
        if (!loader.hasOwnProperty('callbacks') || !Validator.isValid(loader.callbacks)) {
            ...
            console.warn(classDef.name + ' has an invalid property "callbacks". Will change to "Callbacks"');
            loader.callbacks = new Callbacks();
